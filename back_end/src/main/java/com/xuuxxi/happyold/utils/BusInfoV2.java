...
public class BusInfoV2 {
    ...
    private static String getKey(String t) {
        return EncodeByMD5(uname + key + t);
    }

    private static String getFlag(String cityname, String keywords) {
        String url = "http://api.wxbus163.cn/z_busapi/BusApi.php?optype=tfrone&uname=" + uname + "&keySecret=" + getKey("tfrone") + "&cityname=" + cityname + "&keywords=" + keywords;
        JSONObject res = JSONObject.parseObject(restTemplate.getForObject(url, String.class));

        String flag;
        try {
            assert res != null;
            flag = res.getJSONArray("returl_list").getJSONObject(0).getString("imlgn");
        } catch (Exception e) {
            flag = "err";
        }
    ...
    public static JSONObject getAnsInfo(String city, String origin, String destination) {
        String originFlag = getFlag(city, origin);
        String destinationFlag = getFlag(city, destination);

        if (Objects.equals(originFlag, "err") || Objects.equals(destinationFlag, "err"))
            return JSONObject.parseObject("err");

        String url = "http://api.wxbus163.cn/z_busapi/BusApi.php?optype=tfrtwo&uname=" + uname + "&keySecret=" + getKey("tfrtwo") + "&stasta=" + origin + "&staimlgn=" + originFlag + "=&endsta=" + destination + "&endimlgn=" + destinationFlag;

        return JSONObject.parseObject(restTemplate.getForObject(url, String.class));
    }

    public static JSONObject getBusInfo(String city, String keywords){
        if (!arr.contains(city)) return JSONObject.parseObject("city wrong");
        int cityId = Integer.parseInt(arr.get(arr.indexOf(city) + 1));

        //String url = "http://api.wxbus163.cn/z_busapi/BusApi.php?optype=luxian&uname=" + uname + "&cityid=" + cityId + "&keywords=" + keywords + "&keySecret=" + getKey("luxian");
        String url = "http://api.wxbus163.cn/z_busapi/BusApi.php?optype=luxian&uname=" + uname + "&cityid=" + cityId + "&keywords=" + keywords + "&keySecret=" + getKey("luxian");
        return JSONObject.parseObject(restTemplate.getForObject(url, String.class));
    }

    public static JSONObject getBusPos(String city, String busLineId, String busLineNum, String busLineName) {
        if (!arr.contains(city)) return JSONObject.parseObject("city wrong");

        int cityId = Integer.parseInt(arr.get(arr.indexOf(city) + 1));

        String url = "http://api.wxbus163.cn/z_busapi/BusApi.php?optype=rtbus&uname=" + uname + "&cityid=" + cityId + "&bus_linestrid=" + busLineId + "=&bus_linenum=" + busLineNum + "&bus_staname=" + busLineName + "&keySecret=" + getKey("rtbus");
        return JSONObject.parseObject(restTemplate.getForObject(url, String.class));
    }
}
